...
public final class ImageUtils {
     ...
     * @param x 起始横坐标
     * @param y 起始纵坐标
     * @param destWidth 截取图片的宽度
     * @param destHeight 截取图片的高度
     */
    public static void cutImage(File srcFile, File destFile,int x, int y, int destWidth,int destHeight) throws Exception {
        Assert.notNull(srcFile);
        Assert.notNull(destFile);
        if (type == Type.jdk) {
            ...
            int srcHeight = bi.getHeight(); // 源图高度            
                if (srcWidth >= destWidth && srcHeight >= destHeight) {  
                    ...
                    graphics = tag.getGraphics();  
                    graphics.drawImage(img.getScaledInstance(destWidth, destHeight, Image.SCALE_AREA_AVERAGING), 0, 0, null); // 绘制缩小后的图  
                    graphics.dispose();  
                    // 输出为文件  
